...
function onCreate()
	...
	makeLuaSprite('bgThing', 'songBG5', -550, 410)
    ...
    if count < 15 then
        makeLuaText('songText', "".. useName.. " - ".. toughness, 400, getProperty('bgThing.x') + 180, 520) -- 30 letters is the width limit
        ...
        makeIt = true
        makeLuaText('songText', "".. useName, 200, getProperty('bgThing.x') + 180, 500) -- Too big song name
        setObjectCamera("songText", 'other');
        setTextColor('songText', '0x'..songTxtColor)
        setTextSize('songText', 40);
        addLuaText("songText");
    ...
    makeLuaText('beforeSongText', "Now Playing... ", 500, getProperty('bgThing.x') + 100 - 40, 460)
    ...
    if showCredit == true then
        makeLuaText('whoDuntIt', "Made by - ".. name, 400, getProperty('bgThing.x') + 180, 575)
